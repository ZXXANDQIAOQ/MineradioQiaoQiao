...
var SEARCH_HISTORY_MODES = ['song', 'netease', 'qq', 'kugou', 'kuwo', 'migu', 'qishui', 'podcast'];
// 搜索页的标签顺序（与 #search-mode-tabs 里的按钮一一对应）
var SEARCH_MODE_TAB_KEYS = ['song', 'netease', 'qq', 'kugou', 'kuwo', 'migu', 'qishui', 'podcast'];
var SEARCH_MODE_PLACEHOLDERS = {
  song: '搜索歌曲、歌手...',
  netease: '搜索网易云音乐...',
  qq: '搜索 QQ 音乐...',
  kugou: '搜索酷狗音乐...',
...
function updateSearchModeTabs() {
  SEARCH_MODE_TAB_KEYS.forEach(function (mode) {
    var btn = document.getElementById('search-mode-' + mode);
    if (!btn) return;
    btn.classList.toggle('active', searchMode === mode);
    btn.setAttribute('aria-selected', searchMode === mode ? 'true' : 'false');
  });
  if ($input) $input.placeholder = SEARCH_MODE_PLACEHOLDERS[searchMode] || SEARCH_MODE_PLACEHOLDERS.song;
  requestAnimationFrame(updateSearchPillGlassDisplacementMap);
}
function setSearchMode(mode) {
  mode = SEARCH_MODE_TAB_KEYS.indexOf(mode) >= 0 ? mode : 'song';
...
function songProviderKey(song) {
  ...
  if (song && (song.provider === 'kuwo' || song.source === 'kuwo' || song.type === 'kuwo')) return 'kuwo';
  if (song && (song.provider === 'migu' || song.source === 'migu' || song.type === 'migu')) return 'migu';
...
function songSourceTagHtml(song, opts) {
  ...
  var key = /^(netease|qq|kugou|kuwo|migu|qishui|spotify)$/.test(String(rawKey || '')) ? String(rawKey) : songProviderKey(song);
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'kuwo' ? 'KW' : (key === 'migu' ? 'MG' : (key === 'qishui' ? 'QS' : (key === 'spotify' ? 'SP' : 'NE')))));
  if (opts.switcher) {
...
function controlSourceProviders() {
  return [
    ...
    { key: 'kuwo', label: 'KW', title: '酷我' },
    { key: 'migu', label: 'MG', title: '咪咕' },
...
// Mineradio 的搜索 provider → LX 音源标识（LX 搜索层只认 wy/tx/kg/kw/mg）
var SEARCH_PROVIDER_LX_SOURCE = { netease: 'wy', qq: 'tx', kugou: 'kg', kuwo: 'kw', migu: 'mg' };
function searchProviderLxSource(provider) {
  return SEARCH_PROVIDER_LX_SOURCE[provider] || '';
}
function controlSourceSearchUrl(provider, query) {
  var lxSource = searchProviderLxSource(provider);
  if (lxSource) return '/api/lx/search?source=' + lxSource + '&keywords=' + encodeURIComponent(query) + '&limit=8';
...
var MUSIC_SEARCH_PROVIDER_ORDER = ['netease', 'qq', 'kugou', 'kuwo', 'migu', 'qishui'];
...
function searchProviderCanSearch(provider) {
  ...
  return provider === 'netease' || provider === 'qq' || provider === 'kugou' || provider === 'kuwo' || provider === 'migu' || provider === 'qishui';
}
function searchModeProvider(mode) {
  return MUSIC_SEARCH_PROVIDER_ORDER.indexOf(mode) >= 0 ? mode : '';
}
...
function searchProviderUrl(provider, q, limit, offset) {
  ...
  // 网易云 / QQ / 酷狗 / 酷我 / 咪咕 统一走 LX 搜索层（desktop/lx-search，移植自 lx-music-mobile）
  var lxSource = searchProviderLxSource(provider);
  if (lxSource) return '/api/lx/search?source=' + lxSource + '&keywords=' + encodeURIComponent(q) + suffix;
...
function mergeSongSearchResults(neteaseSongs, qqSongs, kugouSongs, qishuiSongs, spotifySongs, kuwoSongs, miguSongs, limit, q) {
  ...
  (kuwoSongs || []).forEach(function (song, i) { push(song, i); });
  (miguSongs || []).forEach(function (song, i) { push(song, i); });
...
async function fetchMusicSearchResults(q, mode, previousPages) {
  ...
  var pageLimitByProvider = { netease: 18, qq: 12, kugou: 12, kuwo: 12, migu: 12, qishui: 12, spotify: 10 };
  ...
  var songsByProvider = { netease: [], qq: [], kugou: [], kuwo: [], migu: [], qishui: [], spotify: [] };
  ...
  var songs = mergeSongSearchResults(
    ...
    songsByProvider.kuwo,
    songsByProvider.migu,
