// ====================================================================
//  lx-preload.js  —  LX 自定义音源脚本运行环境（preload）
//
//  来源：lx-music-mobile (Apache-2.0) 的
//        android/app/src/main/assets/script/user-api-preload.js
//  原版由 QuickJS + Android 原生提供 __lx_native_call__* 能力；
//  这里原样保留脚本侧实现，由 Node 侧（sandbox-worker.js）提供同名的
//  __lx_native_call__* 宿主函数，因此 lx 的 API 契约与移动端完全一致。
//
//  脚本可见的 API：
//    globalThis.lx = { EVENT_NAMES, request, send, on, utils,
//                      currentScriptInfo, version, env }
//  lx.version = 2.0.0
// ====================================================================
...
globalThis.lx_setup = (key, id, name, description, version, author, homepage, rawScript) => {
  ...
  const supportActions = {
    // Mineradio 扩展：上游 lx-music-mobile 只给 musicUrl（歌词/封面只对 local 开放），
    // 这里把 lyric / pic 也对音乐平台放行，让音源脚本可以顺带提供歌词与封面。
    // 脚本不声明这两个动作时行为与上游完全一致。
