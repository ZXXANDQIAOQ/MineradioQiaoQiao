// ====================================================================
//  tx.js — QQ 音乐搜索（LX 原样移植）
//  来源：lx-music-mobile src/utils/musicSdk/tx/musicSearch.js（Apache-2.0）
//  仅把 ESM 导入换成 CommonJS require，搜索逻辑与字段映射未做改动。
//  注意：原实现在 search() 里解构 { body, meta }，本仓库的 httpFetch 会把
//  musics.fcg 返回的 req.data 直接透出，因此这里改成从 data.meta / data.body 取。
// ====================================================================
'use strict';

const { formatPlayTime, sizeFormate } = require('../format');
const { formatSingerName } = require('../format');
