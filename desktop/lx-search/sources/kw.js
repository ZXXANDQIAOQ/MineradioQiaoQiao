// ====================================================================
//  kw.js — 酷我搜索（LX 原样移植）
//  来源：lx-music-mobile src/utils/musicSdk/kw/musicSearch.js（Apache-2.0）
//  仅把 ESM 导入换成 CommonJS require，搜索逻辑与字段映射未做改动。
//  酷我 r.s 接口有时返回单引号伪 JSON，这里用 coerceJson 统一成对象。
// ====================================================================
'use strict';

const { httpFetch } = require('../http');
const { formatPlayTime, decodeName, formatSinger, coerceJson } = require('../format');
...
module.exports = {
  ...
  search(str, page = 1, limit, retryNum = 0) {
    ...
    return this.musicSearch(str, page, limit).then(({ body: rawResult }) => {
      const result = coerceJson(rawResult)
      // console.log(result)
