...
- **新增自动换音源试播**：某首歌曲在当前平台取不到可播放地址（或地址拿到了但媒体起不来）时，按 `网易云 → QQ → 酷狗 → 酷我 → 咪咕` 的顺序自动换到别的平台试，每换一个先等 0.5 秒，第一个能取到链接的就用它播，并把队列里的条目换成那个平台的同名版本、提示「已自动换音源」。候选优先用搜索时顺手留下的同曲其它平台版本（零额外请求），没留到才按「歌名 + 歌手」补搜一次；全部失败则还原队列条目，交回既有兜底与提示流程。实现见 `public/js/modules/05-playback/11a-lx-source-scan.js`，说明见 `docs/ONLINE_SEARCH.md`。
