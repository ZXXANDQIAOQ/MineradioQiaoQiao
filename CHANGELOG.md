...
- 新增自定义音源：可导入 LX 格式的音源脚本，接管播放取链与歌词，支持关闭 / 兜底 / 优先三种模式；脚本在独立线程沙箱中运行，使用说明见 `docs/CUSTOM_SOURCE.md`。
- 控制台「系统」页新增「自定义音源」分组。
- 修复自定义音源的在线导入：改为面板内置输入框（Electron 渲染进程不支持 `window.prompt`，原先点击无反应），并自动把 GitHub / Gitee 代码页链接纠正为 raw 直链、识别「返回网页而非脚本」的链接。
- 更新检查与打包发布地址改为本仓库（`ZXXANDQIAOQ/MineradioQiaoQiao`），两者保持一致；本仓库还没有 Release 时，更新检查会安静地停在当前版本，不会误报。
- 默认启用「LX 音源模式」：界面与播放逻辑保持 Mineradio 原样，播放内容（取链 / 歌词 / 封面）交给导入的 LX 音源脚本，自定义音源默认档位由「兜底」改为「优先使用」。
- 在线搜索改用 LX 音源搜索层（移植 `lx-music-mobile` 的五个内置音源搜索）：网易云 / QQ / 酷狗 / 酷我 / 咪咕，界面仍是 Mineradio 自己的搜索页与标签栏，新增 KW（酷我）与 MG（咪咕）两枚标签，「综合」聚合五个平台；使用说明见 `docs/ONLINE_SEARCH.md`。酷我 / 咪咕没有内置取链，播放依赖自定义音源脚本。
- LX 音源模式下 QQ音乐、网易云、汽水音乐、酷狗音乐的**登录入口整体下线**：顶部账号按钮、启动登录引导、首页「登录同步歌单」磁贴、播放链路的「去登录」提示都不再通向登录；首页磁贴改为「导入 LX 音源」，试听横幅的入口改为「去导入音源」。
- 修复自定义音源取链被静默跳过：播放链路判断「音源是否就绪」时比状态行多剥了一层状态字段，界面显示「已就绪」但取链从未真正发生 —— 结果是酷我 / 咪咕（没有内置取链）搜出来点了放不了，网易云 / 酷狗仍走内置接口而不是音源。现在两处判断一致，跳过时也会在控制台写明原因。
- 修复自定义音源的音质档位：Mineradio 的 `standard / exhigh / lossless / hires` 会先翻译成音源脚本认识的 `128k / 320k / flac / flac24bit`，再按脚本声明的音质表逐级退让，不再出现 `level=undefined`。
- LX 音源模式下综合搜索不再包含汽水音乐（它的播放能力依赖已经下线的登录入口），标签栏里的 QS 也一并收起，避免搜出点了放不了的歌。
- LX 音源模式下播放失败时不再提示「没有其它已登录平台可接管」，改为直说「没有从音源取到可播放地址，可以换个音源脚本或换一首同名版本」。
- **新增自动换平台试播**：某首歌曲在当前播放平台取不到可播放地址（或地址拿到了但媒体起不来）时，按 `网易云 → QQ → 酷狗 → 酷我 → 咪咕` 的顺序自动换到别的平台试，每换一个先等 0.5 秒，第一个能取到链接的就用它播，并把队列里的条目换成那个平台的同名版本、提示「已自动切换平台」。**换的是播放平台，音源脚本始终是同一个**（只是让它拿另一个平台的 id 去取链），控制台会打印在试哪一个平台。候选优先用搜索时顺手留下的同曲其它平台版本（零额外请求），没留到才按「歌名 + 歌手」补搜一次；全部失败则还原队列条目，交回既有兜底与提示流程。实现见 `public/js/modules/05-playback/11a-lx-platform-scan.js`，说明见 `docs/ONLINE_SEARCH.md`。
